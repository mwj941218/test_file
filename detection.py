...
class detectionTest(unittest.TestCase):
    ...
    def test_case1(self):
        ...
        name = str(rows[0])
        # name = "宝5"
        assert "育学园教育" in self.driver.title
    ...
    def test_case2(self):
        ...
        sql = 'SELECT image,skip_url FROM yxy_edu.operational_location_content WHERE operational_location_id = (SELECT id FROM yxy_edu.operational_location WHERE id IN (SELECT operational_location_id FROM yxy_edu.view_operational_location WHERE view_code ="XYSDSD343GHGH") AND TYPE =1 AND STATUS = 0 LIMIT 1) AND STATUS =0'
        print(sql)
        cur.execute(sql)
        rows = cur.fetchone()
        image = str(rows[0])
        skip_url =str(rows[1])
        print("banner图片+跳转链接:" + image +' '+skip_url)
